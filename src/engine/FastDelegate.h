...
template<class RetType=detail::DefaultVoid, typename... Arguments>
class FastDelegateV {
...
private:
	...
	typedef DesiredRetType (*StaticFunctionPtr)(Arguments... args);

	typedef RetType (*UnvoidStaticFunctionPtr)(Arguments... args);
...
public:
	...
	void operator = (const FastDelegateV &x)  {
        clear ();
        mLambda = x.mLambda;
        if ( mLambda )
            bind(&mLambda, &Lambda<RetType (Arguments...)>::operator() );
	...
	bool operator ==(const FastDelegateV &x) const {
        if ( mLambda )
            return mLambda == x.mLambda;
        else
	...
	FastDelegateV(Y *pthis, DesiredRetType (X::* function_to_bind)(Arguments... args) ) {

		m_Closure.bindmemfunc(detail::implicit_cast<X*>(pthis), function_to_bind); }

	template < class X, class Y >

	inline void bind(Y *pthis, DesiredRetType (X::* function_to_bind)(Arguments... args)) {
	...
	FastDelegateV(const Y *pthis, DesiredRetType (X::* function_to_bind)(Arguments... args) const) {

		m_Closure.bindconstmemfunc(detail::implicit_cast<const X*>(pthis), function_to_bind);	}

	template < class X, class Y >

	inline void bind(const Y *pthis, DesiredRetType (X::* function_to_bind)(Arguments... args) const) {
	...
	FastDelegateV(DesiredRetType (*function_to_bind)(Arguments... args) ) {

		bind(function_to_bind);	}

    void operator= ( const Lambda<RetType (Arguments...)>& lambda )
    {
        mLambda = lambda;
        bind(&mLambda, &Lambda<RetType (Arguments...)>::operator() );
    }
	...
	void operator = (DesiredRetType (*function_to_bind)(Arguments... args) ) {
		bind(function_to_bind);	}

	inline void bind(DesiredRetType (*function_to_bind)(Arguments... args)) {

		m_Closure.bindstaticfunc(this, &FastDelegateV::InvokeStaticFunction, 
...
class FastDelegate< R ( Args... ) >
  ...
  : public FastDelegateV < R, Args... >
{
public:
  // Associated lambda
  typedef Lambda<R (Args...)> LambdaType;

  // Make using the base type a bit easier via typedef.
  typedef FastDelegateV < R, Args... > BaseType;
  ...
  FastDelegate ( const Lambda<R (Args...)>& lambda ) : BaseType(lambda) {}

  template < class X, class Y >
  FastDelegate(Y * pthis, 
    R (X::* function_to_bind)( Args... args ))
    : BaseType(pthis, function_to_bind)  { }

  template < class X, class Y >
  FastDelegate(const Y *pthis,
      R (X::* function_to_bind)( Args... args ) const)
    : BaseType(pthis, function_to_bind)
  { }

  FastDelegate(R (*function_to_bind)( Args... args ))
...
FastDelegate<FASTDLGT_RETTYPE (Args...)> MakeDelegate(Y* x, RetType (X::*func)(Args... args)) { 
	return FastDelegate<FASTDLGT_RETTYPE (Args...)>(x, func);
}

template <class X, class Y, class RetType, typename... Args>
FastDelegate<FASTDLGT_RETTYPE (Args...)> MakeDelegate(Y* x, RetType (X::*func)(Args... args) const) { 
	return FastDelegate<FASTDLGT_RETTYPE (Args...)>(x, func);
}
